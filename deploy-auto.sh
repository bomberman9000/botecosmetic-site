...
# Автоматический деплой с новым SSH ключом
...
if [ ! -f "$SSH_KEY" ]; then
    ...
    echo "   Сначала запустите: ./setup-new-key.sh"
    exit 1
fi
...
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no deploy.tar.gz "$VPS_HOST:/tmp/"

if [ $? -eq 0 ]; then
    echo "✅ Архив загружен"
else
    echo "❌ Ошибка загрузки. Возможно, ключ не добавлен на сервер."
    echo "   Запустите: ./setup-new-key.sh"
    exit 1
fi
echo ""
